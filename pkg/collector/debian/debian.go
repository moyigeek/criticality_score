...
func updateOrInsertDatabase(pkgInfoMap map[string]PackageInfo) error {
	...
	for pkgName, pkgInfo := range pkgInfoMap {
		...
		if !exists {
			_, err := db.Exec("INSERT INTO debian_packages (package, depends_count, description, homepage, version) VALUES ($1, $2, $3, $4, $5)",
				pkgName, pkgInfo.DependsCount, pkgInfo.Description, pkgInfo.Homepage, pkgInfo.Version)
			if err != nil {
				return err
			}
		} else {
			_, err := db.Exec("UPDATE debian_packages SET depends_count = $1, description = $2, homepage = $3, version = $4 WHERE package = $5",
